...
public class SessionHostInner extends ProxyResource {
    ...
    /*
     * Status for a SessionHost. Available - Session Host has passed all the
     * health checks and is available to handle connections. Unavailable -
     * Session Host is either turned off or has failed critical health checks
     * which is causing service not to be able to route connections to this
     * session host. Note this replaces previous 'NoHeartBeat' status. Shutdown
     * - Session Host is shutdown - RD Agent reported session host to be
     ...
     * unavailable because currently an upgrade of RDAgent/side-by-side stack
     * is in progress. Note: this state will be removed once the upgrade
     * completes and the host is able to accept connections. UpgradeFailed -
     * Session Host is unavailable because the critical component upgrade
    ...
    /**
     * Get the status property: Status for a SessionHost. Available - Session Host has passed all the health checks and
    ...
    /**
     * Set the status property: Status for a SessionHost. Available - Session Host has passed all the health checks and
     * is available to handle connections. Unavailable - Session Host is either turned off or has failed critical health
     * checks which is causing service not to be able to route connections to this session host. Note this replaces
     * previous 'NoHeartBeat' status. Shutdown - Session Host is shutdown - RD Agent reported session host to be stopped
     * or deallocated. Disconnected - The Session Host is unavailable because it is currently disconnected. Upgrading -
