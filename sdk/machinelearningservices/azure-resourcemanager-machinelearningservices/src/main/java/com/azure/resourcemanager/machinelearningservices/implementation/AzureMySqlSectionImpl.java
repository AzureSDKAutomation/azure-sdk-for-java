// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.machinelearningservices.implementation;

import com.azure.resourcemanager.machinelearningservices.MachinelearningservicesManager;
import com.azure.resourcemanager.machinelearningservices.fluent.models.AzureMySqlSectionInner;
import com.azure.resourcemanager.machinelearningservices.fluent.models.DatastoreCredentialsInner;
import com.azure.resourcemanager.machinelearningservices.models.AzureMySqlSection;
import com.azure.resourcemanager.machinelearningservices.models.DatastoreCredentials;

public final class AzureMySqlSectionImpl implements AzureMySqlSection {
    private AzureMySqlSectionInner innerObject;

    private final MachinelearningservicesManager serviceManager;

    AzureMySqlSectionImpl(AzureMySqlSectionInner innerObject, MachinelearningservicesManager serviceManager) {
        this.innerObject = innerObject;
        this.serviceManager = serviceManager;
    }

    public DatastoreCredentials credentials() {
        DatastoreCredentialsInner inner = this.innerModel().credentials();
        if (inner != null) {
            return new DatastoreCredentialsImpl(inner, this.manager());
        } else {
            return null;
        }
    }

    public String databaseName() {
        return this.innerModel().databaseName();
    }

    public String endpoint() {
        return this.innerModel().endpoint();
    }

    public int portNumber() {
        return this.innerModel().portNumber();
    }

    public String serverName() {
        return this.innerModel().serverName();
    }

    public AzureMySqlSectionInner innerModel() {
        return this.innerObject;
    }

    private MachinelearningservicesManager manager() {
        return this.serviceManager;
    }
}
