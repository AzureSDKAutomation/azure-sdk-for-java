// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.openai.models;

import com.azure.resourcemanager.openai.fluent.models.UsageListResultInner;
import java.util.List;

/** An immutable client-side representation of UsageListResult. */
public interface UsageListResult {
    /**
     * Gets the value property: The list of usages for OpenAI account.
     *
     * @return the value value.
     */
    List<Usage> value();

    /**
     * Gets the inner com.azure.resourcemanager.openai.fluent.models.UsageListResultInner object.
     *
     * @return the inner object.
     */
    UsageListResultInner innerModel();
}
