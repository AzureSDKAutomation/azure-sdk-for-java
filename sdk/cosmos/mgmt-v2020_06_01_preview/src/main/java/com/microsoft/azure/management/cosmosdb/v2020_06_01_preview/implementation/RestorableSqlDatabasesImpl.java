...
class RestorableSqlDatabasesImpl extends WrapperImpl<RestorableSqlDatabasesInner> implements RestorableSqlDatabases {
    ...
    @Override
    public Observable<RestorableSqlDatabaseGetResult> getAsync(String location, String instanceId, String restorableSqlDatabaseId) {
        RestorableSqlDatabasesInner client = this.inner();
        return client.getAsync(location, instanceId, restorableSqlDatabaseId)
        .flatMap(new Func1<RestorableSqlDatabaseGetResultInner, Observable<RestorableSqlDatabaseGetResult>>() {
            @Override
            public Observable<RestorableSqlDatabaseGetResult> call(RestorableSqlDatabaseGetResultInner inner) {
                if (inner == null) {
                    return Observable.empty();
                } else {
                    return Observable.just((RestorableSqlDatabaseGetResult)wrapModel(inner));
                }
            }
       });
    }
