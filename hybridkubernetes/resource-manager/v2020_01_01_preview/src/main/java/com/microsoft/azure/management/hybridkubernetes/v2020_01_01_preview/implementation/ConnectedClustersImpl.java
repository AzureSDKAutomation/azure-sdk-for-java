...
class ConnectedClustersImpl extends GroupableResourcesCoreImpl<ConnectedCluster, ConnectedClusterImpl, ConnectedClusterInner, ConnectedClustersInner, KubernetesManager>  implements ConnectedClusters {
    ...
    @Override
    public Observable<CredentialResult> listClusterUserCredentialAsync(String resourceGroupName, String clusterName) {
        ConnectedClustersInner client = this.inner();
        return client.listClusterUserCredentialAsync(resourceGroupName, clusterName)
        .map(new Func1<CredentialResultInner, CredentialResult>() {
            @Override
            public CredentialResult call(CredentialResultInner inner) {
                return new CredentialResultImpl(inner, manager());
            }
        });
    }

    @Override
    public Observable<ConnectedClusterProxyProfile> listProxyConnectionDetailAsync(String resourceGroupName, String clusterName) {
        ConnectedClustersInner client = this.inner();
        return client.listProxyConnectionDetailAsync(resourceGroupName, clusterName)
        .map(new Func1<ConnectedClusterProxyProfileInner, ConnectedClusterProxyProfile>() {
            @Override
            public ConnectedClusterProxyProfile call(ConnectedClusterProxyProfileInner inner) {
                return new ConnectedClusterProxyProfileImpl(inner, manager());
            }
        });
    }
